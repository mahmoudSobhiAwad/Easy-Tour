...
class TripHistoryOfGeneratedTrips extends StatelessWidget {
  const TripHistoryOfGeneratedTrips({super.key,required this.height,required this.width,required this.currentTrip,required this.finished,required this.upComing});
  final double width;
  final double height;
              ...
              SizedBox(
                ...
                child: ListView.builder(
                    ...
                    itemBuilder: (context,index){
                      return TripHistoryItem(height: height, width: width, tripType: 'Generated Trip With Ai', tripTitle: '${currentTrip[index].title}', leftIcon: const Icon(Icons.circle,color: whatsAppColor,size: 15,),tripDate:currentTrip[index].startDate ,tripDuration:currentTrip[index].durationNum,);
                    }),
              )
            ],
          ),
            ...
            children: [
              ...
              finished.isEmpty?const Center(child: Text('No Completed Trips Try Make One',style: CustomTextStyle.fontBold18,)):SizedBox(
                ...
                child: ListView.builder(
                    ...
                    padding: const EdgeInsets.all(10),
                    itemBuilder: (context,index){
                      return TripHistoryItem(besideTitle: const IconButton(onPressed: null, icon: Icon(Icons.repeat,color: basicColor,)),height: height, width: width, tripType: 'Generated Trips With Ai', tripTitle: '${finished[index].title}', leftIcon: const Icon(Icons.check,color: whatsAppColor,),tripDate: finished[index].startDate,tripDuration: finished[index].durationNum,);
                    }),
              )
            ],
          ),
