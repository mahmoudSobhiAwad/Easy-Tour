...
class TripHistoryOfCustomTrips extends StatelessWidget {
  const TripHistoryOfCustomTrips({super.key,required this.height,required this.width,required this.currentTrip,required this.finished,required this.upComing});
  final double width;
  final double height;
            ...
            children: [
              ...
              upComing.isEmpty?const Center(child: Text('No UpComing Trips Try Make One',style: CustomTextStyle.fontBold18,)):SizedBox(
                ...
                child: ListView.builder(
                    ...
                    itemBuilder: (context,index){
                      return TripHistoryItem(besideTitle: const IconButton(onPressed: null, icon: Icon(Icons.delete,color: closeColor,)),height: height, width: width, tripType: 'Custom Trips', tripTitle: '${upComing[index].title}', leftIcon: const Icon(Icons.access_time_rounded,color: entertainmentColor,),tripDuration: upComing[index].durationNum,tripDate: upComing[index].startDate,);
                    }),
              )
            ],
          ),
            ...
            children: [
              ...
              finished.isEmpty?const Center(child: Text('No Completed Trips Try Make One',style: CustomTextStyle.fontBold18,)):SizedBox(
                ...
                child: ListView.builder(
                    ...
                    padding: const EdgeInsets.all(10),
                    itemBuilder: (context,index){
                      return TripHistoryItem(besideTitle: const IconButton(onPressed: null, icon: Icon(Icons.repeat,color: basicColor,)),height: height, width: width, tripType: 'Custom Trips', tripTitle: '${finished[index].title}', leftIcon: const Icon(Icons.check,color: whatsAppColor,),tripDate: finished[index].startDate,tripDuration: finished[index].durationNum,);
                    }),
              )
            ],
          ),
