import 'package:flutter/material.dart';
import 'package:prepare_project/core/widget/login_sign_up/custom_text_form.dart';
import 'package:prepare_project/features/sign_up/presentation/views/widgets/two_beside_form.dart';
class TravelerData extends StatelessWidget {
  const TravelerData({
    super.key,
    required this.width,
    required this.height,
  });

  final double width;
  final double height;

  @override
  Widget build(BuildContext context) {
    return Column(
      children: [
        CustomTwoFieldForm(
          firstText: 'First Name',
          firstCustomFormField: SizedBox(width: width*0.4,child: const CustomTextFormField(),),
          secondText: 'Last Name',
          secondCustomFormField:SizedBox(width: width*0.4,child: const CustomTextFormField(),),
        ),
        SizedBox(height: height*0.01,),
        SizedBox(
          width: width*0.75,
          child:const CustomTextFormField(label: 'Email',suffix: Icon(Icons.email),),
        ),
        SizedBox(height: height*0.01,),
        CustomTwoFieldForm(
          firstText: 'Gender',
          firstCustomFormField: SizedBox(width: width*0.4,child: const CustomTextFormField(suffix: RotatedBox(
              quarterTurns: 1,
              child: Icon(Icons.arrow_forward_ios)),),),
          secondText: 'Birth Date',
          secondCustomFormField:SizedBox(width: width*0.4,child: const CustomTextFormField(prefix: Icon(Icons.date_range_rounded),),),
        ),
        SizedBox(height: height*0.01,),
        SizedBox(
          width: width*0.6,
          child:const CustomTextFormField(
            label: 'Phone Number',
            prefix: Icon(Icons.phone),
          ),
        ),
      ],
    );
  }
}